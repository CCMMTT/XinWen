<template>
    <div class="man">
		...
		<van-col span="4">
		<van-icon @click="goState1({name:'Index',params:{active1:active1,active:active}})" class="topi" name="arrow-left" color="white" size="0.6rem"/>
		</van-col>
		...
		<van-col span="22" offset="1" class="mzi">
			...
			<p >{{pi.digest}}....</p>
			 <div>
			 	...
			 	<a target="_blank" :href="pi.link">点击跳转到该内容</a>
			 </div>
		</van-col>
	</van-row>
	...
	<van-row class="gbox">
		<van-col span="22" offset="1" class="g" >
			<div ref="a" @click="fhtj({name:'Index',params:{active1:0}})" class="l">推荐</div>
	...
	<van-row class="fool">
		<van-col span="5">
			<van-icon class="fooli" @click="goState({name:'Pinlun',params:{pi:pi,psz:psz,active:active,active1:active1}})" name="chat-o" size="1rem"></van-icon>
		</van-col>
		<van-col span="14">
			<input class="foolipt" @click="goState({name:'Fb',params:{pi:pi,psz:psz,active:active,active1:active1}})" placeholder="我来说俩句..."/>
		</van-col>
		<van-col span="5">
...
<script>
	...
	export default {
		...
		mounted(){
			...
			var s = document.querySelectorAll('.g>div');
			s[this.active1].style.backgroundColor="#e4393c";
		...
		methods:{
			...
			goState(path){
				...
				if (!this.userLoginStatus) {
    			//如果未登录, 提示用户登录
    			this.$router.push({name:'Login',params:{pi:this.pi,psz:this.psz,uname:this.uname,active1:this.active1}})
